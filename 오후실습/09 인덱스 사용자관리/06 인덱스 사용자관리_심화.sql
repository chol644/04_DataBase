...
-- birthYear 칼럼에 대해 idx_usertbl_birthYear 이름의 UNIQUE INDEX를 만드세요(에러가 난다면 이유 설명)
-- 중복값이 있기 때문에 에러 발생: Duplicate entry '1979' for key 'usertbl.idx_usertbl_birthYear'
...
-- 데이터베이스명 : scoula_db, 사용자명 : scoula, 비밀번호 : 1234
-- 접속 호스트 : 제한 없음, scoula_db 데이터베이스(스키마) 생성
